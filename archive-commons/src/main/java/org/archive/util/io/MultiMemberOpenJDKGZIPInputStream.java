...
public class MultiMemberOpenJDKGZIPInputStream  extends OpenJDK7GZIPInputStream {
    ...
    public MultiMemberOpenJDKGZIPInputStream(InputStream in, int size) throws IOException
    {
    ...
    public MultiMemberOpenJDKGZIPInputStream(InputStream in) throws IOException
    {
        // Wrap the stream in a PushbackInputStream...
    ...
    private MultiMemberOpenJDKGZIPInputStream(MultiMemberOpenJDKGZIPInputStream parent) throws IOException
    {
        super(parent.in);
        this.size=-1;
    ...
    private MultiMemberOpenJDKGZIPInputStream(MultiMemberOpenJDKGZIPInputStream parent, int size) throws IOException
    {
    ...
    public int read(byte[] inputBuffer, int inputBufferOffset, int inputBufferLen) throws IOException {
        ...
        if (charsRead==-1)
        {
            ...
            MultiMemberOpenJDKGZIPInputStream child;
            if (this.size==-1)
                child=new MultiMemberOpenJDKGZIPInputStream(this);
            else
                child=new MultiMemberOpenJDKGZIPInputStream(this, this.size);
