...
public class SortedTextFile {
    ...
    private final static Logger LOGGER = Logger.getLogger(SortedTextFile.class.getName());

    protected SeekableLineReaderFactory factory;
    protected int minBlockSize = SeekableLineReaderFactory.BINSEARCH_BLOCK_SIZE;

    public SortedTextFile(SeekableLineReaderFactory factory, int minBlockSize) {
        setFactory(factory);
    ...
    public SortedTextFile(String filename) throws IOException {
        this(filename, SeekableLineReaderFactory.BINSEARCH_BLOCK_SIZE, true);
    }

    public SortedTextFile(String filename, int minBlockSize, boolean useNio)
            throws IOException {
    ...
    public CloseableIterator<String> getRecordIterator(final long offset)
            throws IOException {
    ...
    public long binaryFindOffset(SeekableLineReader slr, final String key,
        ...
        while (max - min > 1) {
            ...
            if (mid > 0)
                slr.skipLine(); // probably a partial line
            line = slr.readLine();
    ...
    public long[] getStartEndOffsets(SeekableLineReader slr, String start,
        ...
        if ((start != null) && !start.isEmpty()) {
            startOffset = this
                    .searchOffset(slr, start, true, defaultComparator);
        }

        return new long[] { startOffset, endOffset };
    }

    // public CloseableIterator<String> getSplitIterator(long startOffset, long
    // endOffset, int numSplits) throws IOException
    // {
    // SeekableLineReader slr = factory.get();
    // return new StepSeekingIterator(slr, startOffset, endOffset, numSplits);
    // }

    public CloseableIterator<String> getSplitIterator(String start, String end,
    ...
    // end exclusive
    public String[] getNthSplit(String start, String end, int split,
            int numSplits) throws IOException {
        SeekableLineReader slr = null;
        String startLine = null;
        String endLine = null;
    ...
    public String getLastLine(SeekableLineReader slr) throws IOException {
        ...
        do {
            ...
            slr.seek(slr.getSize() - lastLineLenTest); // TODO: assume larger
                                                       // buffer
    ...
    class StepSeekingIterator implements CloseableIterator<String> {
        ...
        public StepSeekingIterator(SeekableLineReader slr, long startOffset,
                long endOffset, int numSplits) throws IOException {
            this.slr = slr;
            this.currSplit = 0;
        ...
        public String next() {
            ...
            try {
                ...
                currSplit++;
                long seekDiff = ((endOffset - startOffset) * currSplit)
                        / numSplits;
    ...
    private long searchOffset(SeekableLineReader slr, final String key,
            boolean lessThan, Comparator<String> comparator) throws IOException {
    ...
    private CloseableIterator<String> search(SeekableLineReader slr,
            final String key, boolean lessThan, Comparator<String> comparator)
            throws IOException {
    ...
    public static class CachedStringIterator implements
        ...
        public CachedStringIterator(SeekableLineReader slr, String first,
                String second) {
            this.slr = slr;
            this.first = first;
            this.second = second;
