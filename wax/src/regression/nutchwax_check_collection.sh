...
# and then test that all the urls can be successfully queries with
# exacturl: and collection: keywords.
#
# Requirements:
#	Java 1.5
...
# Regression tests always use the same collection name
...
do
	...
	for URL in $ARCURLS
	do	
		QUERY="$DEPLOYED_WAR_URL$QUERY_URL$EXACTURL_STRING$URL%20$COLLECTION_STRING$COLLECTION_NAME"
		...
		if [ $? -ne 0 ]; then
			echo "Failure searching for exacturl:$URL \ 
			      collection:$COLLECTION_NAME" 1>&2
			echo "Looking for 1 match, output: $RES" 1>&2
			exit 1
		fi
		QUERY="$DEPLOYED_WAR_URL$RSS_URL$EXACTURL_STRING$URL%20$COLLECTION_STRING$COLLECTION_NAME"
