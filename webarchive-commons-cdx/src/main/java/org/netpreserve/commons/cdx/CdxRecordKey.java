...
public class CdxRecordKey implements Comparable<CdxRecordKey> {
    ...
    private StringValue uriKey;

    private StringValue timeStamp;
    ...
    public CdxRecordKey(final String uriKey, final String timeStamp) {
        ...
        this.timeStamp = StringValue.valueOf(Objects.requireNonNull(timeStamp));
    }

    public CdxRecordKey(final StringValue uriKey, final StringValue timeStamp) {
    ...
    public String toString() {
        if (data != null) {
            ...
            return uriKey.toString() + ' ' + timeStamp.toString();
        }
    }
    ...
    public int length() {
        if (data == null) {
            return uriKey.toString().length() + timeStamp.toString().length() + 1;
        } else {
            return data.length;
        }
    }
    ...
    private void parse() {
        for (int i = 0; i < data.length; i++) {
            if (data[i] == ' ') {
                ...
                timeStamp = StringValue.valueOf(data, i + 1, data.length);
                return;
            }
        }
