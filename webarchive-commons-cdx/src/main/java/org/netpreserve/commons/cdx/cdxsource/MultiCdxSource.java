...
public class MultiCdxSource implements CdxSource {
    ...
    @Override
    public SearchResult search(SearchKeyTemplate key, List<Processor> processors, boolean reverse) {
    ...
    @Override
    public long count(SearchKeyTemplate key) {
        long count = 0L;
        for (int i = 0; i < sources.size(); i++) {
    ...
    private class MultiCdxIterable extends AbstractSearchResult {
        ...
        MultiCdxIterable(SearchResult[] sourceIterables, List<Processor> processors, boolean reverse) {
